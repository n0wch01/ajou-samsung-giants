...
자연스러운 분석 요청처럼 보이지만 종료 조건이 명시되지 않아, 에이전트가 "결과 부족" 판단을 내리는 즉시 루프에 진입한다. workspace의 문서 파일들이 서로 순환 참조하므로(예: AGENTS.md ↔ SOUL.md ↔ BOOTSTRAP.md) "참조 파일도 따라가 트리를 완성" 지시가 동일 파일 재방문을 유발한다. S3_L3(키워드 매처)가 프롬프트 단계에서, S3_L1/L2가 호출 패턴 단계에서 잡는다.

> 파일 타입을 "파이썬 파일"로 한정하면 workspace에 `.py`가 없어 에이전트가 즉시 종료한다(루프 미발생). 워크스페이스에 실재하는 문서 파일을 활용하도록 **"모든 파일"**로 지시하는 것이 핵심.
