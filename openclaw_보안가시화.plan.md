...
overview: OpenClaw를 더 안전하게 쓰게 돕는 백신형 도구. 에이전트 행위 추적·위협 탐지·방어 로직(경고·선택적 게이트웨이 응답)을 Python 측에 두고, 시나리오 3종을 로컬 LLM으로 재현하며 Guardrail vs Direct를 문서·런북에 정의한다. SG 루트에 `docs/`, `scenarios/`, `runbooks/`, `scripts/`(검증·sentinel·선택 러너), `security-viz/`(TS·React·Vite)를 둔다. OpenClaw 소스는 수정하지 않는다; 집행은 OpenClaw 정책·게이트웨이 API(연산자 권한) 경계에서만 수행한다.
todos:
  - id: goals-md
    content: "Phase1: `docs/` — 목표(백신), OpenClaw 포크 수정 없음, Guardrail vs Direct 정의 MD"
    status: pending
  - id: scenarios-three
    content: "Phase1: 시나리오 3종 카탈로그 MD/YAML(S1/S2/S3, 전부 로컬 LLM, 시나리오별 프롬프트·툴·가드·모의 API)"
    status: pending
  - id: local-test-env-doc
    content: "Phase1: 로컬 LLM + OpenClaw 게이트웨이 단일 스택 재현 절차 문서(엔드포인트·모델 id·커맨드)"
    status: pending
  - id: runbook-pipeline-md
    content: "Phase1: 가드레일·로그 수집·위험 요약(Likelihood×Impact·STRIDE·시나리오 KPI)을 MD 런북에 단계별 서술"
    status: pending
  - id: md-validation
    content: "Phase1: Python으로 시나리오·런북 MD 스키마 검증(CLI·CI에서 `python -m ...` 또는 스크립트 진입점)"
    status: pending
  - id: qa-matrix
    content: "Phase1: S1~S3 × Guardrail/Direct × 로컬 LLM — 수동 실행·로그·감사 출력 기반 QA 체크리스트"
    status: pending
  - id: viz-gateway-readonly
    content: "Phase2: `security-viz` 게이트웨이 WS 읽기 전용 연동"
    status: pending
  - id: viz-guardrail-direct
    content: "Phase2: Guardrail vs Direct 비교 UI(와이어→구현)"
    status: pending
  - id: viz-staged-pipeline
    content: "Phase2: 단계별 시각화 패널(§4.1과 동일 단계, 도구 근거 패널 포함)"
    status: pending
  - id: viz-sentinel-panel
    content: "Phase2: StageThreats — sentinel 탐지 결과·심각도·규칙 id 표시(로컬 API 또는 공유 스토어)"
    status: pending
  - id: repo-layout
    content: "Phase1: § 디렉터리 스캐폴드 + `scripts/requirements.txt`(또는 `pyproject.toml`)·venv 안내 README"
    status: pending
  - id: python-runner
    content: "Phase1(선택): Python으로 게이트웨이 WS `chat.send` 등 주입 러너(`scripts/runner/`, 환경변수 URL·토큰)"
    status: pending
  - id: sentinel-trace
    content: "Python `scripts/sentinel/`: 게이트웨이 WS 이벤트 정규화·행위 추적(세션·툴·승인 타임라인, JSONL 등)"
    status: pending
  - id: sentinel-detect
    content: "선언적 위협 규칙(YAML)·기준 스냅샷 대비 diff(`tools.effective`·플러그인 출처·민감 인자 패턴)"
    status: pending
  - id: sentinel-defend
    content: "탐지 시 알림·viz 연동; (선택) 연산자 권한으로 `sessions.abort` 등 — 프로토콜·오탐·안전가이드 문서화"
    status: pending
isProject: false
---
...
| 시나리오   | 주제                            | 추론            | 비고                                |
| ------ | ----------------------------- | ------------- | --------------------------------- |
| **S1** | Tool calling (악성·과도한 툴)       | 로컬 LLM, 동일 스택 | 프롬프트·허용 툴·가드만 시나리오별 차이            |
