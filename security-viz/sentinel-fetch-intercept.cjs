/**
 * Sentinel fetch interceptor — CommonJS (--require 형식)
 *
 * NODE_OPTIONS="--require /path/to/this.cjs" openclaw start
 * 으로 로드하면 openclaw 프로세스의 모든 fetch 호출을 가로챈다.
 *
 * 환경 변수:
...
// localhost / 127.0.0.1 등 내부 도메인은 제외 (수집기 재귀 방지 + 내부 통신 보호)
...
async function gateRegisterAndWait(id, meta) {
  ...
  while (Date.now() < deadline) {
    ...
    if (status === "unknown") {
      const err = new Error(`[Sentinel] fetch-gate 인지 손실(서버 재시작 등): ${id}`);
